...
float mathsRadiusOfIcosahedron(float fVolume) {
	return powf(fVolume / ((5 * (3 + sqrt(5))) / 12), 0.33f);
}
...
float actualMathsRadiusOfComeFromVolume(float fVolume, unsigned int coneHeightInXRadius) {
	return powf((fVolume * 3.0f) / (coneHeightInXRadius * cs_fPi), 0.33f);
}
